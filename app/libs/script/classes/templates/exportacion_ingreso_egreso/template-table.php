                ...
                <tr data-id="<?php if (isset($rows[$pk])) echo $rows[$pk]; ?>" id="pdocrud-row-<?php echo $rowcount; ?>" class="pdocrud-data-row <?php if (isset($rows[0]["class"])) echo $rows[0]["class"]; ?>" <?php if (isset($rows[0]["style"])) echo $rows[0]["style"]; ?>>
                    ...
                    ?>
                        <td class="pdocrud-row-actions">
                            <a href="javascript:;" class="btn btn-warning btn-sm descargar_excel" title="Descargar Excel" data-fechacorte="<?=date("Y-m-d", strtotime($rows["fecha_corte"]))?>"  data-estado="<?=strip_tags($rows["tipo_exportacion"])?>" data-folio="<?=str_replace("#", "", $rows[$pk]);?>"><i class="fa fa-file-excel-o text-white"></i> <i class="fas fa-arrow-left text-white"></i> </a>
                        </td>
                    <?php } ?>
                </tr>
            <?php
